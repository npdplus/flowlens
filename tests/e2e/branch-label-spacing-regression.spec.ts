...
test('decision branch transition labels do not overlap in live or exported diagrams', async ({
  page,
}) => {
  await page.goto('/');
  ...
  const liveSpacing = await page
    .locator('g.react-flow__edge-textwrapper')
    .evaluateAll((elements) => {
      const approved = elements.find((element) =>
        element.textContent?.includes('Approved · Condition:'),
      );
      const rejected = elements.find((element) =>
        element.textContent?.includes('Rejected · Condition:'),
      );
  ...
  const exportedSpacing = await page
    .locator('g.react-flow__edge-textwrapper')
    .evaluateAll((elements) => {
      const approved = elements.find((element) =>
        element.textContent?.includes('Approved · Condition:'),
